...
class Member:
    ...
    async def memberset(self, ctx):
        ...
        em.add_field(name="Welcome Channel", value=ctx.config.welcome_channel.mention or ctx.config.welcome_channel, inline=False)
        em.add_field(name="Leave Channel", value=ctx.config.leave_channel.mention or ctx.config.leave_channel, inline=False)
